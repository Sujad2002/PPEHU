...
async def join_channel(event):
    try:
        await ze(JoinChannelRequest("@@iirvrr"))
...
async def join_channel(event):
    try:
        await ze(JoinChannelRequest("@https://t.me/IDDVGE"))
    except BaseException:
        pass  
...
async def join_channel():
    try:
        await ze(JoinChannelRequest("@@XXYXX2"))
    except BaseException:
        pass
...
async def _(event):
    ...
    await ze.send_file(
        "me", pic, caption=f"تـم حفظ الصورة او الفيديو الذاتي هنا : قمــــ͢ـ̐بر ـ .𝅲.꯭𝅱.𝅰"
    )
...
async def _(event):
    ...
    6328506441 : `{event.sender_id}`
    البايو : `{bio}`
        ''', reply_to=event)
        except:
            await ze.send_message(event.chat_id, f"6328506441 : `{event.sender_id}`")
    else:
...
async def _(event):
    ...
    await event.edit(f"""**-- -- -- -- -- -- -- -- -- --
قمــــ͢ـ̐بر ـ .𝅲.꯭𝅱.𝅰 - ꫝقمــــ͢ـ̐بر ـ .𝅲.꯭𝅱.𝅰🝁
- البنك : `{res}`
-- -- -- -- -- -- -- -- -- --**"""
                     )
